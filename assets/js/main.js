const userCreatePost =  document.getElementById("usercreatepost")
const createPost = document.querySelector(".user-post");


function userPosts(){
    const post = getLsData("fbPosts");
    let content = "";
    if (post.length > 0) {
    post.reverse().map((item, index)=>{
        content +=`
        
        <div class="single-item my-3">
...
userCreatePost.onsubmit=(e)=>{
    e.preventDefault();
    const form_data= new FormData(userCreatePost);
    const data =  Object.fromEntries(form_data);

    if (!data.name || !data.profile_image) {
        alert(`please input name and profile image`);
    }else{
// get data ls
const prevData = getLsData("fbPosts");
prevData.push({
    author_name : data.name,
    author_photo : data.profile_image,
    post_image: data.post_image ?? null,
    post_content: data.content ?? null,
    post_time : Date.now() 
})
// sent data ls
sendLsData("fbPosts", prevData);
